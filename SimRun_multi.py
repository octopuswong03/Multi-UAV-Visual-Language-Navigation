...
# Some imported modules parse argv at import-time. Preserve CLI args for our own parser,
# and shield third-party imports from unknown flags.
...
from src.llm.prompt_builder import (
    route_planning_prompt_builder,
    route_planning_multievidence_prompt_builder,
    parse_viewpoint_response_v2,
    parse_viewpoint_response_multievidence,
)
...
def pose_to_xyz(pose) -> List[float]:
    """Convert pose-like input to [x, y, z].

    Supports:
    - airsim.Pose
    - numpy array / list / tuple with at least 3 values ([x, y, z, ...])
    """
    if hasattr(pose, "position"):
        return [float(pose.position.x_val), float(pose.position.y_val), float(pose.position.z_val)]
    ...
    if isinstance(pose, np.ndarray):
        ...
        raise TypeError(f"Unsupported pose type for pose_to_xyz: {type(pose)}")

    if arr.size < 3:
        raise ValueError(f"pose_to_xyz expects at least 3 values, got shape={arr.shape}")
    return [float(arr[0]), float(arr[1]), float(arr[2])]


def _compact_blackboard_item(h: Dict[str, Any]) -> Dict[str, Any]:
    out = {
        "landmark_idx": int(h.get("landmark_idx", -1)),
        "landmark_name": str(h.get("landmark_name", "unknown")),
        "executor": str(h.get("executor", "unknown")),
        "sem_conf": float(h.get("sem_conf", 0.0)),
        "vlm_raw": float(h.get("vlm_raw", 0.0)),
        "npts": int(h.get("npts", 0)),
    }
    pos = h.get("pos_mu", None)
    if isinstance(pos, np.ndarray) and pos.shape[0] >= 3:
        out["pos_mu"] = [float(pos[0]), float(pos[1]), float(pos[2])]
    return out
...
def _build_multi_agent_context(
    ...
    blackboard: Optional["TeamBlackboard"] = None,
    max_agents: int = 3,
) -> Dict[str, Any]:
    self_pose = curr_poses.get(agent_name, None)
    if self_pose is None:
        return {}

    self_xy = np.array([self_pose.position.x_val, self_pose.position.y_val], dtype=np.float32)
    teammates = []
    for n, p in curr_poses.items():
        if n == agent_name:
            continue
        xy = np.array([p.position.x_val, p.position.y_val], dtype=np.float32)
        dist_m = float(np.linalg.norm(xy - self_xy))
        teammates.append({
            "agent": n,
            "pose_xyz": pose_to_xyz(p),
            "dist_m": dist_m,
        })
    teammates.sort(key=lambda x: x["dist_m"])
    teammates = teammates[:max(0, int(max_agents))]

    out = {
        "self_agent": agent_name,
        "self_pose_xyz": pose_to_xyz(self_pose),
        "teammates": teammates,
    }
    if blackboard is not None and blackboard.pending is not None:
        out["pending_hypothesis"] = _compact_blackboard_item(blackboard.pending)
    return out
# ==================== 2) 指标记录器（可选） ====================
...
class CoNavConfig:
    ...
    # memory exploit gate (prevents too-early exploit takeover)
    enable_memory_exploit: bool = True
    memory_exploit_min_iter: int = 5
    memory_exploit_min_landmarks_done: int = 2
    memory_exploit_node_thresh_m: float = 12.0
    memory_exploit_min_steps_remaining: int = 10
    # prompt-level multi-evidence fusion (zero-shot)
    enable_multievidence_prompt: bool = True
    llm_score_point_scale: float = 60.0   # convert LLM prior score to "pseudo points" for candidate ranking
    temporal_traj_window: int = 15
    temporal_landmark_window: int = 6
    multi_uav_context_limit: int = 3
    # thresholds
...
def explore_pipeline_by_sam(
    ...
    semantic_context = {
        "next_landmark": landmarks_route[next_landmark_idx],
        "traversed_landmarks": traversed_landmarks[-6:],
        "remaining_landmarks": landmarks_route[next_landmark_idx: next_landmark_idx + 6],
        "scene_objects_hint": list(scene_objects[:30]) if scene_objects is not None else [],
    }
    if isinstance(evidence_context.get("semantic"), dict):
        semantic_context.update(evidence_context.get("semantic", {}))

    spatial_context = {
        "current_pose_xyz": pose_to_xyz(curr_pose),
        "viewpoint_pose_xyz": {vp: pose_to_xyz(viewpoint_poses[vp]) for vp in obs_viewpoint},
        "viewpoints": obs_viewpoint,
    }
    temporal_context = evidence_context.get("temporal", {})
    collaborative_context = evidence_context.get("multi_agent", {})

    if cfg.enable_multievidence_prompt:
        route_predict_prompt = route_planning_multievidence_prompt_builder(
            navigation_instruction,
            landmarks_route,
            traversed_landmarks,
            landmarks_route[next_landmark_idx],
    ...
    try:
        if cfg.enable_multievidence_prompt:
            route_plan = parse_viewpoint_response_multievidence(route_predicted)
        else:
            raise ValueError("legacy_parser")
    except Exception:
        try:
            route_predicted_dict = parse_viewpoint_response_v2(route_predicted)
            route_plan = {
                "is_found": bool(route_predicted_dict.get("is_found", False)),
                "viewpoint_objects": {vp: str(route_predicted_dict.get(vp, "unknown")) for vp in obs_viewpoint},
                "viewpoint_scores": {vp: 0.0 for vp in obs_viewpoint},
                "next_landmark_prob": {vp: 0.0 for vp in obs_viewpoint},
                "evidence_alignment": {},
            }
        except Exception as e:
    ...
    if route_plan.get("is_found", False):
        next_subgoal_found = True

    # --- build semantic point cloud candidates ---
    candidates = []  # (vp, obj, pc, n_points, vlm_raw, llm_prior)
    seg_succ_all = False

    for vp in obs_viewpoint:
        obj = route_plan.get("viewpoint_objects", {}).get(vp, "unknown")
        if not isinstance(obj, str):
            continue
        obj = obj.strip()
        if obj == "" or obj.lower() == "unknown":
            continue

        llm_prior = float(max(
            route_plan.get("viewpoint_scores", {}).get(vp, 0.0),
            route_plan.get("next_landmark_prob", {}).get(vp, 0.0),
        ))
        ...
        if len(semantic_part_pc) > 0:
            candidates.append((vp, obj, semantic_part_pc, int(len(semantic_part_pc)), float(vlm_raw), llm_prior))

    # deterministic pick: align geometric support + LLM evidence score, then viewpoint priority
    best_npts = 0
    best_llm_prior = 0.0
    ...
    if len(candidates) > 0:
        ...
        point_scale = float(max(0.0, getattr(cfg, "llm_score_point_scale", 0.0)))
        candidates.sort(key=lambda x: (-(x[3] + point_scale * x[5]), -x[3], -x[5], vp_priority.get(x[0], 99)))
        best_vp, best_obj, semantic_pc, best_npts, best_vlm_raw, best_llm_prior = candidates[0]
        print(f"[PC select] use {best_vp}/{best_obj}, points={best_npts}, vlm_raw={best_vlm_raw:.3f}, llm_prior={best_llm_prior:.3f}")

    # choose a "preferred" viewpoint from LLM output, used for fallback motion
    preferred_vp = None
    preferred_obj = None
    vp_priority = {"front": 0, "slightly_right": 1, "slightly_left": 2, "right": 3, "left": 4}
    llm_pref = []
    for vp in obs_viewpoint:
        obj = route_plan.get("viewpoint_objects", {}).get(vp, "unknown")
        if not isinstance(obj, str) or obj.strip() == "" or obj.strip().lower() == "unknown":
            continue
        score = float(max(
            route_plan.get("viewpoint_scores", {}).get(vp, 0.0),
            route_plan.get("next_landmark_prob", {}).get(vp, 0.0),
        ))
        llm_pref.append((score, vp, obj.strip()))
    if len(llm_pref) > 0:
        llm_pref.sort(key=lambda x: (-x[0], vp_priority.get(x[1], 99)))
        preferred_vp, preferred_obj = llm_pref[0][1], llm_pref[0][2]
...
# --- Belief observation (mu/cov/conf) ---
    ...
    pc_bundle = {
        ...
        "llm_prior": float(best_llm_prior),
        "llm_prob": route_plan.get("next_landmark_prob", {}),
        "llm_viewpoint_scores": route_plan.get("viewpoint_scores", {}),
...
def MultiUAVCityNavAgent(
    ...
    for i in tqdm(range(len(navi_tasks))):
        ...
        confirmed_landmarks: List[Dict[str, Any]] = []
        ...
        def _build_temporal_context(agent_name: str, step_idx: int) -> Dict[str, Any]:
            return {
                "step_index": int(step_idx),
                "agent_recent_trajectory": data_dict["pred_traj_team"].get(agent_name, [])[-cfg.temporal_traj_window:],
                "confirmed_landmarks_recent": confirmed_landmarks[-cfg.temporal_landmark_window:],
                "blackboard_history_recent": [_compact_blackboard_item(h) for h in blackboard.history[-3:]],
            }
        ...
        while global_iter < max_step_size and next_landmark_idx < len(landmarks):
            ...
            if cfg.enable_verification and blackboard.pending is not None:
                ...
                if verifier is None:
                    ...
                    if isinstance(hyp.get("pos_mu", None), np.ndarray) and hyp["pos_mu"].shape[0] >= 3:
                        cxyz = [float(hyp["pos_mu"][0]), float(hyp["pos_mu"][1]), float(hyp["pos_mu"][2])]
                    else:
                        cxyz = pose_to_xyz(curr_poses[executor])
                    confirmed_landmarks.append({
                        "landmark_idx": int(next_landmark_idx),
                        "landmark_name": lm_name,
                        "pos_xyz": cxyz,
                        "source": "auto_confirm_no_verifier",
                ...
                if ok_dist and ok_conf:
                    ...
                    confirmed_landmarks.append({
                        "landmark_idx": int(next_landmark_idx),
                        "landmark_name": lm_name,
                        "pos_xyz": [float(mu[0]), float(mu[1]), float(mu[2])],
                        "source": "executor_verifier",
                        "iter": int(global_iter),
                    })
                            ...
                            if cfg.enable_search_during_regroup and n == agent_names[0] and next_landmark_idx < len(landmarks):
                                ...
                                for k in range(0, len(mid_coords), stride):
                                    ...
                                    sz2, new_pose2, found2, obs_mu2, cov2, geom2, pc_bundle2 = explore_pipeline_by_sam(
                                        ...
                                        evidence_context={
                                            "temporal": _build_temporal_context(n, global_iter),
                                            "multi_agent": _build_multi_agent_context(
                                                n, curr_poses, blackboard, max_agents=cfg.multi_uav_context_limit
                                            ),
                                        }
                                    )
            ...
            for n in agent_names:
                ...
                if n == agent_names[0] and cfg.enable_memory_exploit:
                    remaining_budget = int(step_budget - max(step_used.values()))
                    enough_progress = (
                        global_iter >= int(cfg.memory_exploit_min_iter)
                        and next_landmark_idx >= int(cfg.memory_exploit_min_landmarks_done)
                    )
                    has_budget = remaining_budget >= int(cfg.memory_exploit_min_steps_remaining)
                    cls_node = None
                    if enough_progress and has_budget:
                        cls_node = find_closest_node(
                            mem_graph._graph,
                            list(curr_poses[n].position),
                            thresh=float(cfg.memory_exploit_node_thresh_m),
                        )
                    ...
                    if cls_node is not None:
                        ...
                        if int(sz_mem) <= 0:
                            print(f"[CO-UAV][{n}] memory exploit skipped: non-positive planned steps (sz_mem={sz_mem}). Continue explore.")
                            continue

                        rest_steps = int(min(max(0, remaining_budget), int(sz_mem)))
                        if rest_steps <= 0:
                            print(f"[CO-UAV][{n}] memory exploit skipped: no positive budget left (remaining={remaining_budget}). Continue explore.")
                            continue
            ...
            if (not cfg.enable_verification) and (found_executor is not None):
                ...
                if found_hyp is not None and isinstance(found_hyp.get("pos_mu", None), np.ndarray) and found_hyp["pos_mu"].shape[0] >= 3:
                    cxyz = [float(found_hyp["pos_mu"][0]), float(found_hyp["pos_mu"][1]), float(found_hyp["pos_mu"][2])]
                else:
                    cxyz = pose_to_xyz(curr_poses[found_executor])
                confirmed_landmarks.append({
                    "landmark_idx": int(next_landmark_idx),
                    "landmark_name": lm_name,
                    "pos_xyz": cxyz,
                    "source": "ablation_auto_confirm",
                    "iter": int(global_iter),
                })
                next_landmark_idx += 1
                blackboard.clear()
...
def CityNavAgent(scene_id, split, data_dir="./data", max_step_size=200, vlm_name="dino", record=False,
    ...
    for i in tqdm(range(len(navi_tasks))):
        ...
        single_traj_history: List[List[float]] = [pose_to_xyz(curr_pose)]
        confirmed_landmarks_single: List[Dict[str, Any]] = []
        ...
        while step_size < max_step_size:
            ...
            # explore
                ...
                sz, new_pose, next_landmark_found, obs_mu, *_ = explore_pipeline_by_sam(
                    curr_pose, llm, vlm,
                    cfg_single,
                    pano_obs_imgs_path,
                    pano_obs_imgs,
                    ...
                    evidence_context={
                        "temporal": {
                            "step_index": int(step_size),
                            "agent_recent_trajectory": single_traj_history[-cfg_single.temporal_traj_window:],
                            "confirmed_landmarks_recent": confirmed_landmarks_single[-cfg_single.temporal_landmark_window:],
                        },
                        "multi_agent": {},
                    }
                )
                ...
                curr_pose = new_pose
                single_traj_history.append(pose_to_xyz(curr_pose))
                step_size += sz
                hist_step_size.append(sz)

                if next_landmark_found:
                    confirmed_landmarks_single.append({
                        "landmark_idx": int(next_landmark_idx),
                        "landmark_name": landmarks[next_landmark_idx],
                        "pos_xyz": [float(obs_mu[0]), float(obs_mu[1]), float(obs_mu[2])],
                        "source": "single_llm_vlm",
                        "iter": int(step_size),
                    })
                    next_landmark_idx += 1
...
def _build_argparser():
    ...
    p.add_argument("--no_memory_exploit", action="store_true")
    p.add_argument("--memory_exploit_min_iter", type=int, default=5)
    p.add_argument("--memory_exploit_min_landmarks_done", type=int, default=2)
    p.add_argument("--memory_exploit_node_thresh", type=float, default=12.0)
    p.add_argument("--memory_exploit_min_steps_remaining", type=int, default=10)
    ...
    p.add_argument("--legacy_prompt", action="store_true", help="Use legacy prompt without structured multi-evidence context")
    p.add_argument("--llm_score_point_scale", type=float, default=60.0, help="Scale factor that converts LLM evidence score to pseudo-points for candidate ranking")
    p.add_argument("--temporal_traj_window", type=int, default=15)
    p.add_argument("--temporal_landmark_window", type=int, default=6)
    p.add_argument("--multi_uav_context_limit", type=int, default=3)
    p.add_argument("--verbose", action="store_true")
    return p

if __name__ == '__main__':
    args = _build_argparser().parse_args(_ORIGINAL_ARGV[1:])
    ...
    if args.mode == "single":
        ...
        cfg = CoNavConfig(
            ...
            enable_memory_exploit=(not args.no_memory_exploit),
            memory_exploit_min_iter=args.memory_exploit_min_iter,
            memory_exploit_min_landmarks_done=args.memory_exploit_min_landmarks_done,
            memory_exploit_node_thresh_m=args.memory_exploit_node_thresh,
            memory_exploit_min_steps_remaining=args.memory_exploit_min_steps_remaining,
            enable_multievidence_prompt=(not args.legacy_prompt),
            llm_score_point_scale=args.llm_score_point_scale,
            temporal_traj_window=args.temporal_traj_window,
            temporal_landmark_window=args.temporal_landmark_window,
            multi_uav_context_limit=args.multi_uav_context_limit,
